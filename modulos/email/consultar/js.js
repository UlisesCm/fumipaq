...
function comprobarReglas(){
	...
	if(recuperarCookie("campoOrdenMarca")!=null){
		campoOrden=recuperarCookie("campoOrdenMarca");
	...
	if(recuperarCookie("cantidadamostrarMarca")!=null){
		cantidadamostrar=recuperarCookie("cantidadamostrarMarca");
	...
	if(recuperarCookie("ordenMarca")=="asc"){
		orden="ASC"
		$('#asc').attr('checked', true);
		$('#desc').attr('checked', false);
	}else if(recuperarCookie("ordenMarca")=="desc"){
		orden="DESC"
	...
	if(recuperarCookie("mostrarNombreMarca")=="si"){
		$('.Cnombre').show();
		$('#CheckNombre').attr('checked', true);
	}else if(recuperarCookie("mostrarNombreMarca")=="no"){
		$('.Cnombre').hide();
		$('#CheckNombre').attr('checked', false);
	}
	//Mostrar u Ocultar Idmarca
	if(recuperarCookie("mostrarIdmarcaMarca")=="si"){
		$('.Cidmarca').show();
		$('#CheckIdmarca').attr('checked', true);
	}else if(recuperarCookie("mostrarIdmarcaMarca")=="no"){
		$('.Cidmarca').hide();
		$('#CheckIdmarca').attr('checked', false);
	}
	//Mostrar u Ocultar Composicion
	if(recuperarCookie("mostrarComposicionMarca")=="si"){
		$('.Ccomposicion').show();
		$('#CheckComposicion').attr('checked', true);
	}
	if(recuperarCookie("mostrarComposicionMarca")=="no"){
		$('.Ccomposicion').hide();
		$('#CheckComposicion').attr('checked', false);
	}
	
	//Elegir el tipo de vista
	if(recuperarCookie("tipoVistaMarca")=="tabla"){
		$('.tipoLista').show();
		$('.tipoTabla').hide();
		tipoVista="tabla";
	}else{
	...
	$( ".tipoTabla" ).click(function() {
    	crearCookie("tipoVistaMarca", "tabla");
		tipoVista="tabla";
		$(".tipoLista").show();
		$(".tipoTabla").hide();
	...
	$( ".tipoLista" ).click(function() {
    	crearCookie("tipoVistaMarca", "lista");
		tipoVista="lista";
		$(".tipoLista").hide();
		$(".tipoTabla").show();
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
...
$(document).ready(function() {
	...
	$("#campoOrden").change(function(){
		...
		crearCookie("campoOrdenMarca", campoOrden);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$("#cantidadamostrar").change(function(){
		cantidadamostrar = this.value;
		crearCookie("cantidadamostrarMarca", cantidadamostrar);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$( "#asc" ).click(function() {
    	if ($( "#asc" ).is(':checked')){
			crearCookie("ordenMarca", "asc");
			orden="ASC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	...
	$( "#CheckNombre" ).click(function() {
    	if ($( "#CheckNombre" ).is(':checked')){
			crearCookie("mostrarNombreMarca", "si");
			$('.Cnombre').show();
		}else{
			crearCookie("mostrarNombreMarca", "no");
			$('.Cnombre').hide();
		}	
	});
	$( "#CheckIdmarca" ).click(function() {
    	if ($( "#CheckIdmarca" ).is(':checked')){
			crearCookie("mostrarIdmarcaMarca", "si");
			$('.Cidmarca').show();
		}else{
			crearCookie("mostrarIdmarcaMarca", "no");
			$('.Cidmarca').hide();
		}	
	});
	$( "#CheckComposicion" ).click(function() {
    	if ($( "#CheckComposicion" ).is(':checked')){
			crearCookie("mostrarComposicionMarca", "si");
			$('.Ccomposicion').show();
		}else{
			crearCookie("mostrarComposicionMarca", "no");
			$('.Ccomposicion').hide();
		}
	});
