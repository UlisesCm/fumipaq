...
if (isset($_POST['idcontacto'])){
	$idcontacto=htmlentities(trim($_POST['idcontacto']));
	//$idcontacto=mysql_real_escape_string($idcontacto);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idcontacto no es correcto</p>";
}
...
if($validacion){
	$resultado=$Ocontacto->actualizar($idcliente,$nombrecontacto,$telefono,$email,$departamento,$comentarios, $idcontacto);
	if($resultado=="exito"){
