...
if (isset($_POST['idplantillamensaje'])){
	$idplantillamensaje=htmlentities(trim($_POST['idplantillamensaje']));
	//$idplantillamensaje=mysql_real_escape_string($idplantillamensaje);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idplantillamensaje no es correcto</p>";
}
...
if($validacion){
	$resultado=$Oplantillamensaje->actualizar($titulo,$asunto,$mensaje, $idplantillamensaje);
	if($resultado=="exito"){
