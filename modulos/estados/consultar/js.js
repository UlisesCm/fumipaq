...
function comprobarReglas(){
	...
	if(recuperarCookie("campoOrdenEstado")!=null){
		campoOrden=recuperarCookie("campoOrdenEstado");
	...
	if(recuperarCookie("cantidadamostrarEstado")!=null){
		cantidadamostrar=recuperarCookie("cantidadamostrarEstado");
	...
	if(recuperarCookie("ordenEstado")=="asc"){
		orden="ASC"
		$('#asc').attr('checked', true);
		$('#desc').attr('checked', false);
	}else if(recuperarCookie("ordenEstado")=="desc"){
		orden="DESC"
	...
	//Mostrar u Ocultar Idestado
	if(recuperarCookie("mostrarIdestadoEstado")=="si"){
		$('.Cidestado').show();
		$('#CheckIdestado').attr('checked', true);
	}else if(recuperarCookie("mostrarIdestadoEstado")=="no"){
		$('.Cidestado').hide();
		$('#CheckIdestado').attr('checked', false);
	}
	//Mostrar u Ocultar Nombre
	if(recuperarCookie("mostrarNombreEstado")=="si"){
		$('.Cnombre').show();
		$('#CheckNombre').attr('checked', true);
	}else if(recuperarCookie("mostrarNombreEstado")=="no"){
		$('.Cnombre').hide();
		$('#CheckNombre').attr('checked', false);
	}
	//Mostrar u Ocultar Composicion
	if(recuperarCookie("mostrarComposicionEstado")=="si"){
		$('.Ccomposicion').show();
		$('#CheckComposicion').attr('checked', true);
	}
	if(recuperarCookie("mostrarComposicionEstado")=="no"){
		$('.Ccomposicion').hide();
		$('#CheckComposicion').attr('checked', false);
	}
	
	//Elegir el tipo de vista
	if(recuperarCookie("tipoVistaEstado")=="tabla"){
		$('.tipoLista').show();
		$('.tipoTabla').hide();
		tipoVista="tabla";
	}else{
	...
	$( ".tipoTabla" ).click(function() {
    	crearCookie("tipoVistaEstado", "tabla");
		tipoVista="tabla";
		$(".tipoLista").show();
		$(".tipoTabla").hide();
	...
	$( ".tipoLista" ).click(function() {
    	crearCookie("tipoVistaEstado", "lista");
		tipoVista="lista";
		$(".tipoLista").hide();
		$(".tipoTabla").show();
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
...
$(document).ready(function() {
	...
	$("#campoOrden").change(function(){
		...
		crearCookie("campoOrdenEstado", campoOrden);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$("#cantidadamostrar").change(function(){
		cantidadamostrar = this.value;
		crearCookie("cantidadamostrarEstado", cantidadamostrar);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$( "#asc" ).click(function() {
    	if ($( "#asc" ).is(':checked')){
			crearCookie("ordenEstado", "asc");
			orden="ASC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	$( "#desc" ).click(function() {
    	if ($( "#desc" ).is(':checked')){
			crearCookie("ordenEstado", "desc");
			orden="DESC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	$( "#CheckIdestado" ).click(function() {
    	if ($( "#CheckIdestado" ).is(':checked')){
			crearCookie("mostrarIdestadoEstado", "si");
			$('.Cidestado').show();
		}else{
			crearCookie("mostrarIdestadoEstado", "no");
			$('.Cidestado').hide();
		}	
	});
	$( "#CheckNombre" ).click(function() {
    	if ($( "#CheckNombre" ).is(':checked')){
			crearCookie("mostrarNombreEstado", "si");
			$('.Cnombre').show();
		}else{
			crearCookie("mostrarNombreEstado", "no");
			$('.Cnombre').hide();
		}	
	});
	$( "#CheckComposicion" ).click(function() {
    	if ($( "#CheckComposicion" ).is(':checked')){
			crearCookie("mostrarComposicionEstado", "si");
			$('.Ccomposicion').show();
		}else{
			crearCookie("mostrarComposicionEstado", "no");
			$('.Ccomposicion').hide();
		}
	});
