...
function comprobarReglas(){
	...
	if(recuperarCookie("campoOrdenActividad")!=null){
		campoOrden=recuperarCookie("campoOrdenActividad");
	...
	if(recuperarCookie("cantidadamostrarActividad")!=null){
		cantidadamostrar=recuperarCookie("cantidadamostrarActividad");
	...
	if(recuperarCookie("ordenActividad")=="asc"){
		orden="ASC"
		$('#asc').attr('checked', true);
		$('#desc').attr('checked', false);
	}else if(recuperarCookie("ordenActividad")=="desc"){
		orden="DESC"
	...
	if(recuperarCookie("mostrarNombreActividad")=="si"){
		$('.Cnombre').show();
		$('#CheckNombre').attr('checked', true);
	}else if(recuperarCookie("mostrarNombreActividad")=="no"){
		$('.Cnombre').hide();
		$('#CheckNombre').attr('checked', false);
	}
	//Mostrar u Ocultar Idmodeloimpuestos
	if(recuperarCookie("mostrarIdmodeloimpuestosActividad")=="si"){
		$('.Cidmodeloimpuestos').show();
		$('#CheckIdmodeloimpuestos').attr('checked', true);
	}else if(recuperarCookie("mostrarIdmodeloimpuestosActividad")=="no"){
		$('.Cidmodeloimpuestos').hide();
		$('#CheckIdmodeloimpuestos').attr('checked', false);
	}
	//Mostrar u Ocultar Tipoformato
	if(recuperarCookie("mostrarTipoformatoActividad")=="si"){
		$('.Ctipoformato').show();
		$('#CheckTipoformato').attr('checked', true);
	}else if(recuperarCookie("mostrarTipoformatoActividad")=="no"){
		$('.Ctipoformato').hide();
		$('#CheckTipoformato').attr('checked', false);
	}
	//Mostrar u Ocultar Composicion
	if(recuperarCookie("mostrarComposicionActividad")=="si"){
		$('.Ccomposicion').show();
		$('#CheckComposicion').attr('checked', true);
	}
	if(recuperarCookie("mostrarComposicionActividad")=="no"){
		$('.Ccomposicion').hide();
		$('#CheckComposicion').attr('checked', false);
	}
	
	//Elegir el tipo de vista
	if(recuperarCookie("tipoVistaActividad")=="tabla"){
		$('.tipoLista').show();
		$('.tipoTabla').hide();
		tipoVista="tabla";
	}else{
	...
	$( ".tipoTabla" ).click(function() {
    	crearCookie("tipoVistaActividad", "tabla");
		tipoVista="tabla";
		$(".tipoLista").show();
		$(".tipoTabla").hide();
	...
	$( ".tipoLista" ).click(function() {
    	crearCookie("tipoVistaActividad", "lista");
		tipoVista="lista";
		$(".tipoLista").hide();
		$(".tipoTabla").show();
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
...
$(document).ready(function() {
	...
	$("#campoOrden").change(function(){
		...
		crearCookie("campoOrdenActividad", campoOrden);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$("#cantidadamostrar").change(function(){
		cantidadamostrar = this.value;
		crearCookie("cantidadamostrarActividad", cantidadamostrar);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$( "#asc" ).click(function() {
    	if ($( "#asc" ).is(':checked')){
			crearCookie("ordenActividad", "asc");
			orden="ASC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	$( "#desc" ).click(function() {
    	if ($( "#desc" ).is(':checked')){
			crearCookie("ordenActividad", "desc");
			orden="DESC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	$( "#CheckNombre" ).click(function() {
    	if ($( "#CheckNombre" ).is(':checked')){
			crearCookie("mostrarNombreActividad", "si");
			$('.Cnombre').show();
		}else{
			crearCookie("mostrarNombreActividad", "no");
			$('.Cnombre').hide();
		}	
	});
	$( "#CheckIdmodeloimpuestos" ).click(function() {
    	if ($( "#CheckIdmodeloimpuestos" ).is(':checked')){
			crearCookie("mostrarIdmodeloimpuestosActividad", "si");
			$('.Cidmodeloimpuestos').show();
		}else{
			crearCookie("mostrarIdmodeloimpuestosActividad", "no");
			$('.Cidmodeloimpuestos').hide();
		}	
	});
	$( "#CheckTipoformato" ).click(function() {
    	if ($( "#CheckTipoformato" ).is(':checked')){
			crearCookie("mostrarTipoformatoActividad", "si");
			$('.Ctipoformato').show();
		}else{
			crearCookie("mostrarTipoformatoActividad", "no");
			$('.Ctipoformato').hide();
		}	
	});
	$( "#CheckComposicion" ).click(function() {
    	if ($( "#CheckComposicion" ).is(':checked')){
			crearCookie("mostrarComposicionActividad", "si");
			$('.Ccomposicion').show();
		}else{
			crearCookie("mostrarComposicionActividad", "no");
			$('.Ccomposicion').hide();
		}
	});
