...
if (isset($_FILES['factura']['name'])){
	...
	$factura=basename($_FILES['factura']['name'],".".$extencionfactura)."_".generarClave(5).".".$extencionfactura;
	
	if($facturatemporal==""){
		$factura="";
		$validacion=false;
		$mensaje=$mensaje."<p>El campo factura es obligatorio</p>";
	}
...
if($validacion){
	$resultado=$Ogarantias->guardar($idempresa,$idsucursal,$fecha,$area,$factura,$descripcion,$estatus,$provedor);
	if($resultado=="exito"){
		/*CARGAR ARCHIVOS*/
		$mensajeArchivo="";
