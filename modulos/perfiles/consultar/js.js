...
function comprobarReglas(){
	...
	if(recuperarCookie("campoOrdenPerfil")!=null){
		campoOrden=recuperarCookie("campoOrdenPerfil");
	...
	if(recuperarCookie("cantidadamostrarPerfil")!=null){
		cantidadamostrar=recuperarCookie("cantidadamostrarPerfil");
	...
	if(recuperarCookie("ordenPerfil")=="asc"){
		orden="ASC"
		$('#asc').attr('checked', true);
		$('#desc').attr('checked', false);
	}else if(recuperarCookie("ordenPerfil")=="desc"){
		orden="DESC"
	...
	if(recuperarCookie("mostrarNombrePerfil")=="si"){
		$('.Cnombre').show();
		$('#CheckNombre').attr('checked', true);
	}else if(recuperarCookie("mostrarNombrePerfil")=="no"){
		$('.Cnombre').hide();
		$('#CheckNombre').attr('checked', false);
	}
	//Mostrar u Ocultar Composicion
	if(recuperarCookie("mostrarComposicionPerfil")=="si"){
		$('.Ccomposicion').show();
		$('#CheckComposicion').attr('checked', true);
	}
	if(recuperarCookie("mostrarComposicionPerfil")=="no"){
		$('.Ccomposicion').hide();
		$('#CheckComposicion').attr('checked', false);
	}
	
	//Elegir el tipo de vista
	if(recuperarCookie("tipoVistaPerfil")=="tabla"){
		$('.tipoLista').show();
		$('.tipoTabla').hide();
		tipoVista="tabla";
	}else{
	...
	$( ".tipoTabla" ).click(function() {
    	crearCookie("tipoVistaPerfil", "tabla");
		tipoVista="tabla";
		$(".tipoLista").show();
		$(".tipoTabla").hide();
	...
	$( ".tipoLista" ).click(function() {
    	crearCookie("tipoVistaPerfil", "lista");
		tipoVista="lista";
		$(".tipoLista").hide();
		$(".tipoTabla").show();
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
...
$(document).ready(function() {
	...
	$("#campoOrden").change(function(){
		...
		crearCookie("campoOrdenPerfil", campoOrden);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$("#cantidadamostrar").change(function(){
		cantidadamostrar = this.value;
		crearCookie("cantidadamostrarPerfil", cantidadamostrar);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$( "#asc" ).click(function() {
    	if ($( "#asc" ).is(':checked')){
			crearCookie("ordenPerfil", "asc");
			orden="ASC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	...
	$( "#CheckNombre" ).click(function() {
    	if ($( "#CheckNombre" ).is(':checked')){
			crearCookie("mostrarNombrePerfil", "si");
			$('.Cnombre').show();
		}else{
			crearCookie("mostrarNombrePerfil", "no");
			$('.Cnombre').hide();
		}	
	});
	$( "#CheckComposicion" ).click(function() {
    	if ($( "#CheckComposicion" ).is(':checked')){
			crearCookie("mostrarComposicionPerfil", "si");
			$('.Ccomposicion').show();
		}else{
			crearCookie("mostrarComposicionPerfil", "no");
			$('.Ccomposicion').hide();
		}
	});
