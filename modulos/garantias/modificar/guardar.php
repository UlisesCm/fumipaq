...
if (isset($_POST['fingarantia'])){
	...
	$validacion=false;
	$mensaje=$mensaje."<p>El campo Fin de Garantia no es correcto</p>";
}
...
if (isset($_FILES['facturaI']['name'])){
	...
	if($facturatemporal==""){
		...
		$factura=basename($_FILES['facturaI']['name'],".".$extencionfactura)."_".generarClave(5);
...
if($validacion){
	$resultado=$Ogarantias->actualizar($idempresa,$idsucursal,$fecha,$area,$factura,$descripcion,$estatus,$provedor, $idgarantia,$fingarantia);
	if($resultado=="exito"){
		/*CARGAR ARCHIVOS*/
		$mensajeArchivo="";
