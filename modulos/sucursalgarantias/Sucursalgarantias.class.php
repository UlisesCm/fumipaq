...
class Sucursalgarantias{
 //constructor	
	...
	function armarConsulta($condicion,$papelera){
		if ($condicion!=""){
			$consulta="WHERE ((sucursalgarantias.nombre LIKE '%".$condicion."%') OR (sucursalgarantias.idsucursal LIKE '%".$condicion."%'))AND sucursalgarantias.idsucursal <>'0'";
		}else{
	...
	function contar($condicion, $papelera){
		...
		if($this->con->conectar()==true){
			$resultado=mysqli_query($this->con->conect,"SELECT 
					...
					INNER JOIN empresasgarantias ON sucursalgarantias.idempresa=empresasgarantias.idempresa
					$where");
	...
	function mostrarFiltro($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera, $idempresa){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['sucursalgarantias']['consultar'])){
			return "denegado";
			exit;
		...
		$consultaEmpresa = "";
		if ($idempresa != "") {
			$consultaEmpresa = " AND empresasgarantias.nombrecomercial='$idempresa' ";
		}

		$condicion= trim($condicion);

		$where = "";
		$where= "WHERE sucursalgarantias.idsucursal<>0
		$consultaEmpresa
		";
