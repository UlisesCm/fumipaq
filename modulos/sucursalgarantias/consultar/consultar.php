...
if (isset($_REQUEST['idempresa']) && $_REQUEST['idempresa'] !="" && $_REQUEST['idempresa'] !="1") {
	if($_REQUEST['idempresa']!="undefined"){
		$idempresa = htmlentities($_REQUEST['idempresa']);
	}else{
		$idempresa="";
	}
}else{
...
if($busqueda == ""){
	$resultado=$Osucursalgarantias->mostrarFiltro($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera, $idempresa);
	$filasTotales=$resultado[1];
	$resultado=$resultado[0];
} else {
	$resultado=$Osucursalgarantias->mostrar($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera);
	$filasTotales=$resultado[1];
	$resultado=$resultado[0];
}
	...
	while ($filas=mysqli_fetch_array($resultado)) { ?>
      		<tr id="iregistro<?php echo $filas['idsucursal'] ?>">
                ...
                <td>
					...
					?>
						<form action="../modificar/actualizar.php?n1=sucursalgarantias&n2=consultarsucursalgarantias" method="post">
