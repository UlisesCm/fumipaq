...
function comprobarReglas(){
	...
	if(recuperarCookie("campoOrdenSucursales")!=null){
		campoOrden=recuperarCookie("campoOrdenSucursales");
	...
	if(recuperarCookie("cantidadamostrarSucursales")!=null){
		cantidadamostrar=recuperarCookie("cantidadamostrarSucursales");
	...
	if(recuperarCookie("ordenSucursales")=="asc"){
		orden="ASC"
		$('#asc').attr('checked', true);
		$('#desc').attr('checked', false);
	}else if(recuperarCookie("ordenSucursales")=="desc"){
		orden="DESC"
	...
	if(recuperarCookie("mostrarComposicionSucursales")=="si"){
		$('.Ccomposicion').show();
		$('#CheckComposicion').attr('checked', true);
	}
	if(recuperarCookie("mostrarComposicionSucursales")=="no"){
		$('.Ccomposicion').hide();
		$('#CheckComposicion').attr('checked', false);
	}
	
	//Elegir el tipo de vista
	if(recuperarCookie("tipoVistaSucursales")=="tabla"){
		$('.tipoLista').show();
		$('.tipoTabla').hide();
		tipoVista="tabla";
	}else{
	...
	$( ".tipoTabla" ).click(function() {
    	crearCookie("tipoVistaSucursales", "tabla");
		tipoVista="tabla";
		$(".tipoLista").show();
		$(".tipoTabla").hide();
	...
	$( ".tipoLista" ).click(function() {
    	crearCookie("tipoVistaSucursales", "lista");
		tipoVista="lista";
		$(".tipoLista").hide();
		$(".tipoTabla").show();
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
...
$(document).ready(function() {
	...
	$("#campoOrden").change(function(){
		...
		crearCookie("campoOrdenSucursales", campoOrden);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$("#cantidadamostrar").change(function(){
		cantidadamostrar = this.value;
		crearCookie("cantidadamostrarSucursales", cantidadamostrar);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$( "#asc" ).click(function() {
    	if ($( "#asc" ).is(':checked')){
			crearCookie("ordenSucursales", "asc");
			orden="ASC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	$( "#desc" ).click(function() {
    	if ($( "#desc" ).is(':checked')){
			crearCookie("ordenSucursales", "desc");
			orden="DESC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	$( "#CheckComposicion" ).click(function() {
    	if ($( "#CheckComposicion" ).is(':checked')){
			crearCookie("mostrarComposicionSucursales", "si");
			$('.Ccomposicion').show();
		}else{
			crearCookie("mostrarComposicionSucursales", "no");
			$('.Ccomposicion').hide();
		}
	});
